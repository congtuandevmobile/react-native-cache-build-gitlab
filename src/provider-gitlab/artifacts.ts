...
async function httpJson<T>(url: string, repo: GitLabRepoDetails): Promise<T> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 401 || res.status === 403) {
      ...
      throw new RockError(
        `GitLab auth failed.\nUpdate token under ${color.bold(
          "remoteCacheProvider"
        )} in ${colorLink("rock.config.mjs")}. Local uses ${color.bold(
          "PRIVATE-TOKEN"
        )}, CI uses ${color.bold("JOB-TOKEN")}.\nURL: ${colorLink(
          url
        )}\nError: ${info}`
      );
    }
...
export async function fetchGitLabArtifactsByName(
  ...
  while (true) {
    const url =
      `${repo.registryServer}/api/v4/projects/${encodeURIComponent(
...
export async function deleteGitLabArtifacts(
  ...
  try {
    ...
    throw new RockError(
      `Failed to delete GitLab artifacts${
        artifactNameForLog ? ` for "${artifactNameForLog}"` : ""
      }`,
      { cause: error }
    );
  }
